...
public class DataInitializer {
    ...
    CommandLineRunner seedData() {
        return args -> {
            ...
            Volunteer volunteer = volunteerRepository.save(Volunteer.builder().user(volunteerUser).skills("Photography, Crowd Management").availability("Weekdays").notes("Strong communication skills").build());
            ...
            upcomingWorkshop.getSessions().addAll(List.of(EventSession.builder().event(upcomingWorkshop).title("Prompt Engineering Bootcamp").description("Writing reliable prompts").sessionDate(LocalDate.now().plusDays(5)).startTime(LocalTime.of(10, 0)).endTime(LocalTime.of(12, 0)).speaker(speaker1).build(), EventSession.builder().event(upcomingWorkshop).title("Build Your First AI Workflow").description("Hands-on implementation").sessionDate(LocalDate.now().plusDays(5)).startTime(LocalTime.of(13, 0)).endTime(LocalTime.of(16, 0)).speaker(speaker2).build()));
            ...
            EventRegistration reg1 = registrationRepository.save(EventRegistration.builder().event(upcomingWorkshop).user(student).status(RegistrationStatus.APPROVED).bookmarked(true).checkedIn(false).registrationDate(LocalDateTime.now().minusDays(1)).remarks("Confirmed seat").build());
